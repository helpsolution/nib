...
Зависимостей нет и здесь: скрипт компилирует `swiftc`-ом модули, работающие без
запущенного приложения (`Prefs`, `Typo`, `Highlighter`, `MarkdownDocument`),
вместе с `Tests/*.swift` и запускает получившийся бинарник. Ни SwiftPM, ни XCTest,
ни Xcode-проекта.
...
| Блоки кода защищены | `**` и `#` внутри ``` — это текст программы, а не разметка |
| Инкрементальный проход равен полному | подсветка красит только затронутый кусок, и она не имеет права расходиться с полным проходом |
| Кодировки | открыл и сохранил без правок — те же байты, для UTF-8, UTF-16, CP1251, KOI8-R, MacRoman |
| Ритм строки | от межстрочного интервала зависит высота каретки |

Самая ценная — сравнение проходов: именно она поймала ошибку, из-за которой
инкрементальная подсветка молча красила не тот диапазон.
