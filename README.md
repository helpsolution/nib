...
- Затухающий синтаксис: `**`, `#`, `[]()` гаснут до серого, содержимое остаётся
  контрастным — разметка читается прямо в тексте.
- Режим просмотра по `⌘R`: тот же документ без разметки — настоящие буллеты,
  нумерация, чекбоксы, горизонтальные линейки и таблицы с сеткой. Панели превью
  по-прежнему нет: режим занимает то же окно и ту же колонку, ничего не делится
  пополам и нечему разъезжаться при прокрутке. Переключатель — бледной парой
  иконок рядом со счётчиком слов, проявляется под курсором.
...
Размер шрифта и ширина колонки запоминаются между запусками. Режим просмотра
запоминается тоже, но только как режим для новых окон: у каждого окна он свой,
так что одну заметку можно читать, пока соседнюю правишь.
...
| `Sources/EditorScreen.swift` | Окно, счётчик слов, переключатель режима, хромировка |
| `Sources/Editor.swift` | Две `NSTextView` — исходник и просмотр — и мост в SwiftUI |
| `Sources/Highlighter.swift` | Регулярки и раскраска разметки в режиме исходника |
| `Sources/Renderer.swift` | Разбор markdown в готовое оформление для режима просмотра |
...
| `Sources/Prefs.swift` | Размер шрифта, ширина колонки и режим в `UserDefaults` |

`build.sh` и `test.sh` подхватывают `Sources/*.swift` по маске — новый файл
достаточно просто положить в папку.
